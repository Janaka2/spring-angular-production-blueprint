...
| **Terraform + hcloud provider** | Describes cloud resources in files and creates them | `infra/hetzner`: the CX33 server, SSH key, cloud firewall, backups | Reproducible: the same server again in ten minutes, reviewed like code. Without it: clicking through the console and forgetting a firewall rule. |
